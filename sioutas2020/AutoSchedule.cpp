...
// helper function which rounds up a num by a multiple, useful for GPU
// calculations
Expr roundUp(const Expr &num, const Expr &multiple) {
...
Expr roundDown(Expr num, const Expr &m) {
    Expr n = cast<int>((std::move(num)));
...
struct DependenceAnalysis {
    ...
    // 'only_regions_computed' is set to true, this only returns the computed
    // regions and not the total allocated regions.
    map<string, Box> regions_required(const Function &f, const DimBounds &pure_bounds,
                                      const set<string> &prods,
                                      bool only_regions_computed,
                                      const Scope<Interval> *input_estimates);
...
map<string, Box> DependenceAnalysis::regions_required(
    const Function &f, const DimBounds &pure_bounds, const set<string> &prods,
...
void merge_and_queue_regions(map<FStage, DimBounds> &fs_bounds,
                             ...
                             bool only_regions_computed, const string &curr_func_name,
                             const set<StageBounds> &visited) {
...
map<string, Box> DependenceAnalysis::regions_required(
    const Function &f, int stage_num, const DimBounds &bounds,
    const set<string> &prods, bool only_regions_computed,
    const Scope<Interval> *input_estimates) {
...
map<string, Box> DependenceAnalysis::redundant_regions(
    const Function &f, int stage_num, const string &var, const DimBounds &bounds,
    const set<string> &prods, bool only_regions_computed,
    const Scope<Interval> *input_estimates) {
...
struct Partitioner {
    ...
    split_dim(const Group &g, Stage f_handle, int stage_num, const Definition &def,
              ...
              const string &in_suffix, const string &out_suffix, map<string, Expr> &estimates,
              AutoSchedule &sched);
...
void Partitioner::get_gpu_params(const Target &target) {
    ...
    internal_assert(target.has_feature(Target::CUDA) || target.has_feature(Target::OpenCL) || target.has_feature(Target::OpenGLCompute));
    if (target.has_feature(Target::CUDACapability30)) {
        ...
        gparams.limit_shared_mem_per_SM = make_const(Float(32), 49152);
        gparams.limit_shared_mem_per_block = make_const(Float(32), 49152);
        gparams.limit_threads_per_SM = make_const(Float(32), 256);//Based on RX580 limits (Max work group 256)
        gparams.limit_threads_per_block = make_const(Float(32), 256);//Based on RX580 limits (Max work group 256)
        gparams.n_SM = arch_params.parallelism;
...
vector<map<string, Expr>> Partitioner::generate_tile_configs(const FStage &stg,
    ...
    if (tile_vars.size() > 1) {
        ...
        while (flag_iter) {
            ...
            // check iter
            a[index]++;
            bool is_thread = thread_vars.find(tile_vars[index]) != thread_vars.end();
            if (is_thread) {
                if (final_tile)
                ...
                if ((thread_vars.size() <= 2 &&
                    ...
                    (thread_vars.size() > 2 && (can_prove(tiles[index] > max_tile))) ||
            ...
            if ((can_prove(extents[tile_vars[index]] > 1024) &&
                 (can_prove(tiles[index] >= extents[tile_vars[index]] / 32))) ||
                (can_prove(extents[tile_vars[index]] < 1024) &&
                 (can_prove(tiles[index] > extents[tile_vars[index]] / 2))))
                a[index] = max_int;
...
set<string> Partitioner::dims_to_tile(const FStage &stg) {
    ...
    // first find which have an extent of at least 16
    DimBounds stg_bounds = get_bounds(stg);
    ...
    for (auto &it1 : tile_vars1) {
        ...
        extents.emplace_back(it1, extent);
    }
...
vector<Expr> Partitioner::estimate_occupancy(const Expr &threads, const Expr &shared_mem,
    ...
    debug(3) << "Estimating regs per block..." << regs_per_block << '\n';
    // group regs_per_SM
    ...
    debug(3) << "Estimating occupancy limit warps per SM..."
             << block_group_warps_per_SM << '\n';
    ...
    debug(3) << "Estimating occupancy limit regs per SM..."
             << block_group_regs_per_SM << '\n';

    Expr block_group_shared_mem_per_SM = simplify(
        roundDown(gparams.limit_shared_mem_per_SM / shared_mem_bytes, 1));

    debug(3) << "Estimating occupancy limit SH mem per SM..."
    ...
    if (can_prove(block_group_warps_per_SM <= block_group_regs_per_SM) &&
        ...
        debug(3) << "Limited by Max Warps or Max Blocks per Multiprocessor" << '\n';
        ...
        debug(3) << "Limited by Registers per Multiprocessor" << '\n';
        active_blocks_per_SM = block_group_regs_per_SM;
    } else {

        debug(3) << "Limited by Shared Memory per Multiprocessor" << '\n';
    ...
    debug(3) << "Estimating active warps..." << active_warps_per_SM << '\n';
    // occupancy
    ...
    debug(3) << "active threads " << simplify(active_threads) << '\n';
    active_blocks = min(std::move(n_blocks) / active_blocks_per_SM, active_blocks_per_SM);

    debug(3) << "active blocks " << simplify(active_blocks) << '\n';
    Expr active_SMs = min(gparams.n_SM, active_blocks * gparams.n_SM);

    debug(3) << "active SMs " << simplify(active_SMs) << '\n';
...
Partitioner::get_bounds_from_tile_sizes(const FStage &s,
    ...
    for (int d = 0; d < (int)dims.size() - 1; d++) {
        ...
        if (iter != tile_sizes.end()) {
            ...
            Expr extent = get_extent(bound);
            if (extent.defined() && can_prove(extent >= 2 * size)) {
...
Partitioner::eval_max_threads(const Group &g, bool show_analysis) {
    ...
    threads.emplace_back(g.output, spawned_threads);
    ...
    for (const auto &stg : local_bounds) {
        ...
        if (is_inlined == g.inlined.end()) {
            ...
            for (const auto &iti : stg_estimates) {
                ...
                if ((dim_in_tiles != thread_vars.end() && (can_prove(extent > 0)))) {
                    if (can_prove(extent / iti.second >= 2)) {
                        auto itiles = max_threads.find(tdim);
                        if (itiles == max_threads.end())
                            max_threads[tdim] = iti.second;
                        else
                            max_threads[tdim] = max(max_threads[tdim], iti.second);

                        spawned_threads_local *= iti.second;
                    } else {
                        auto itiles = max_threads.find(tdim);
                        if (itiles == max_threads.end())
                            max_threads[tdim] = extent;
                        else
                            max_threads[tdim] = max(max_threads[tdim], extent);
            ...
            threads.emplace_back(stg.first, spawned_threads_local);
        }
    }
...
bool access_inputs_or_outputs(const Definition &def, const VarOrRVar &var,
...
Partitioner::split_dim(const Group &g, Stage f_handle, int stage_num,
                       const Definition &def, bool is_group_output, const VarOrRVar &v,
                       const Expr &factor, const string &in_suffix, const string &out_suffix,
                       map<string, Expr> &estimates, AutoSchedule &sched) {
...
void Partitioner::generate_group_cpu_schedule(
    ...
    VarOrRVar tile_inner_var("tile_inner_var", false);
    if (!outer_dims.empty()) {
...
void Partitioner::generate_cpu_schedule(const Target &t, AutoSchedule &sched) {
    ...
    for (const pair<FStage, Group> &g : groups) {
        if (g.second.members.size() - g.second.inlined.size() > 1) {
            ...
            debug(3) << "SH MEM " << asda.shared_mem << '\n';
            debug(3) << " ACT THR " << asda.active_threads << '\n';
            debug(3) << " OCC " << asda.occupancy << '\n';
...
struct Sioutas20 {
    void operator()(const Pipeline &pipeline, const Target &target, const MachineParams &arch_params, AutoSchedulerResults *outputs) {
        ...
        // }
        // auto_data_transform(pipeline_outputs);
